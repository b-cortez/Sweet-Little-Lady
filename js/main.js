class bubble {
    ...
    randomise() {
      this.colour = Math.random() * 755;
      this.size = this.generateDecimalBetween(50, 80);
      this.movementX = this.generateDecimalBetween(-0.4, 0.5);
      this.movementY = this.generateDecimalBetween(0.3, .5);
  ...
  class background {
    ...
    animate() {
      ...
      self.bubblesList.forEach(function(bubble) {
        bubble.update();
        self.ctx.beginPath();
        self.ctx.arc(bubble.posX, bubble.posY, bubble.size, 0, 8 * Math.PI);
    ...
    bubbleDensity() {
      return Math.sqrt((this.canvas.height, this.canvas.width) * .03);
    }
  }
